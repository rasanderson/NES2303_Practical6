...
dune_plt1 <- ordi_plot(dune_pca, display="sites", geom="text", title="PCA with full dataset")
dune_plt2 <- ordi_plot(dune_pca_no6, display="sites", geom="text", title="PCA with one sample omitted")
...
interpret. **Note** When you repeat this exercise using RStudio, the best graph
is from using points (without the `geom="text"` option) and then using
`ordi_identify()` to interactively label the points you are most interested in.
...
commands below we set `ordi_plot()` to `layers=c("sites", "biplot")` or `layers=c("species", "biplot")` where `biplot` represents the "biplot" arrow
points for continuous explanatory variables. If you have **categorical**
explanatory variables replace these with `layers=c("sites", "centroids")` where
the`centroids` represents centre for each level of your category. If you have a
