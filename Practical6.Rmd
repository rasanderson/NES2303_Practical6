...
subtitle: "NES2303 Statistics and experimental design for biologists"
...
terms if needed. Constrained analysis exists in two main forms, linear and unimodal. The linear form is Redundancy Analysis (RDA) and unimodal is Canonical Correspondence Analysis (CCA). These are run using the `rda()` and `cca()` functions respectively. When these functions are called without explanatory variables (Practical 5) they do PCA and CA. When you include explanatory variables they do the closely related RDA and CCA constrained versions of the analyses. There is no constrained equivalent of NMDS.
...
The approach for the analysis and interpretation of linear methods is very similar. At the end of this practical you should have a good understanding of how to handle and interpret multi-variable data from laboratory and field experients in Biology and Zoology.
...
3. Save your script, e.g. call it `practical6.R`
...
```{r, eval=FALSE}
library(vegan) # Activate the vegan library
...
This gives a rather cluttered graph, so you will probably find it easier to plot
the samples and species (plus the environmental constraints) on separate plots:
...
# Add the explanatory variable names, also called biplot (bp) scores
text(varespec_cca, scaling = 0, display = "bp", head.arrow = 0.1,
     lwd = 2, col= "blue")
```

Addding the various components of the plots separately gives you a lot of 
control, e.g. over font size, labelling, arrow size etc. Repeat the process for
the species and biplot scores:
...
### 1.2.1 Interpret the plots - Environmental variables (any graph)
In both your samples+environment and species+environment plots you can see that Aluminium (`Al`), Phosphorus (`P`) and bare soil (`Baresoil`) are the most important variables, as these have the longest arrows. Al and pH are positively correlated with other (arrows point in same direction), and K and P are positively correlated (roughly) as arrows point in the same direction. pH and Baresoil are negatively correlated (arrows point in opposite directions). pH and P are uncorrelated with each other (arrows roughly 90-degrees).

### 1.2.2 Interpret the plots - Samples (Sites and environment graph)
You can see that samples 9, 10, 2, 3 and 4 all have high pH and high Aluminium, as they are near the arrow tips. Samples 21, 22, 16 and 14 have lots of bare soil. Samples 24, 25, 27 and 28 have relatively high P. Note also that samples 5, 6, 7, 13 and 18 have relatively low P, as they are on the **opposite** site of the graph to the P arrow-head. As usual, samples close to each other have similar species composition, samples far apart are dissimilar. If you were analysing data from a molecular biology laboratory experiment with gene up- or down-regulation, then isolates (samples) close to each other would have similar genes up- or down-regulated. You could relate your isolates to any experimental treatments, which would be the equivalent of your environment variables.

### 1.2.3 Interpret the plots - Species (Species and environment graph)
Species Flavniva, Cladcerv, Cladstel at the bottom right of the plot are associated with high pH and high Aluminium, Species Polycomm, Rhodtome, Cladbotr like bare soil. Hylosple and Discrsp like high K and P, whereas Callvulg, Icmaeric and Vacculig like low K and P. Species close to each other in the plot tend to co-occur. Those far apart are rarely found growing together. Likewise, if you were doing a molecular biology experiment, and your data were isolates and gene regulations, then genes close to each other would be up- or down-regulated in similar ways etc.

## 1.3 Simplifying the plot to view samples or species
There is still a lot of information in the plots so you can simplify them to only
label some species or samples. Our problem is that our species plot in particular
is very cluttered as the **ubiquituous species** that occur in lots of samples
overlap in the middle.

We can avoid this problem by storing the plot in a separate R object and then
calling the `identify()` function. **Note**: when you use `identify()` the 
species names are not added until you have hit the "Esc" key (top left of your
keyboard).

```{r, eval=FALSE}
# Species; save in R object and label points with mouse
varespec_cca_spp_plt  <- plot(varespec_cca, type="points", display=c("species", "bp"))
identify(varespec_cca_spp_plt, "species") # Hit Esc to exit
# Call the identify function again if you want to label more species
```

If your plots are too cluttered, this is an easy way of simplifying the plots to make them easier to interpret.

## 1.4 Bare soil is not soil chemistry
The longest arrow (and hence most important explanatory variable) is bare soil which might be affected by other factors e.g. animal grazing / trampling etc. but less affected by soil chemistry. If your main aim is to understand the effects of soil chemistry, and you not interested in bare soil, you need to be able to **remove the effects of bare soil** so that you can focus on the soil chemistry. You cannot simply omit bare soil from the analysis, as you know it's important. Instead you do a **partial** analysis, which answers the question "What is the effect of soil chemistry **after** taking into account any impact of bare soil?" This is easy to do with a **partial constrained analysis**. Simply add the term `Condition(Baresoil)` to your explanatory variables to remove its effect.

```{r, eval=FALSE}
# Partial CCA, storing the results in a new R object
varespec_cca2 <- occa(varespec ~ K + P + Al + pH + Condition(Baresoil), data=varechem)

# If you find these plots too cluttered, use ordi_identify
plot(varespec_cca2, display=c("sites", "bp"))
plot(varespec_cca2, display=c("species", "bp"))
```
...
* $n_x$ is the number of times the shuffled (permutated) data gave the same results as the original unsuffled data
...
# Summary
Constrained ordination, available via either `rda()` for linear analysis or `cca()` for unimodal analysis provides a powerful tool to understand the relationships between your multiple response variables and explanatories. Whilst the process of statistical analysis is slightly different, interpretation of results on graphs and from tables is relatively straightforward.